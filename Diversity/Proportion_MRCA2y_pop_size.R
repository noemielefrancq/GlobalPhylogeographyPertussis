...
#####################################################################################
## Correlation with population size of each region
#####################################################################################
...
#####################################################################################
### Average prop MRCA<2y per region, worldwide (average), without French regions
#####################################################################################
threshold_MRCA = 2
threshold_seqs = 2
...
mat_tmp = (MRCA_mat2<=threshold_MRCA)*geo_mat*time_mat2
mat_tmp[which(mat_tmp == 0)] = NA
mat_tmp2 = geo_mat*time_mat2
mat_tmp2[which(mat_tmp2 == 0)] = NA

boot.out[1] = sum(mat_tmp, na.rm = T)/sum(mat_tmp2, na.rm = T)
...
for(j in 1:nsim){
  ...
  MRCA_mat2 = MRCA_mat[A,A]
  for (i in (1:(nboot))){
    tmp = sample(nseq, nseq, prob = w_vec, replace = T)
...
#####################################################################################
### Average prop MRCA<2y per country, worldwide (average), without French regions
#####################################################################################
...
boot.out = matrix(NA, length(pmid1), nboot + 1)
for(i in 1:length(pmid1)){
  ...
  boot.out[i,1] = sum(mat_tmp, na.rm = T)/sum(mat_tmp2, na.rm = T)
...
##Compute the ci over the 2 to nboot +1 columns 
boot.ci1_worldwide = apply(boot.out[,(2:(nboot+1))], 1, quantile, probs = c(0.025, 0.975), na.rm = T)
boot.ci.m_worldwide = apply(boot.out[,(2:(nboot+1))], 1, quantile, probs = c(0.5), na.rm = T)
boot.out_all[[1]] = boot.out[,(2:(nboot+1))]
